  ...
  const features = [
    {
      ...
      icon: Briefcase,
      color: "from-blue-600 to-zinc-50"
    },
    {
    ...
    },
    {
      ...
      icon: Zap,
      color: "from-zinc-100 to-teal-500"
    }
  ];
  ...
  return (
    <div className="py-8 flex flex-col gap-16 max-w-5xl mx-auto">
      ...
      <section className="text-center flex flex-col items-center gap-6 mt-6">
        {/* Sparkle badge */}
        <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-zinc-100 border border-zinc-300 text-zinc-700 text-[10px] font-bold tracking-widest uppercase shadow-lg shadow-zinc-200 select-none animate-pulse">
          <Sparkles size={12} className="text-zinc-600" />
          <span>Autonomous AI Engineering & Career Suite</span>
        </div>
        
        <h1 className="text-4xl sm:text-6xl font-black tracking-tight leading-[1.1] font-display text-zinc-900 max-w-3xl">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-4 w-full justify-center text-xs">
          <button 
            ...
            className="btn-primary py-3.5 px-8 text-xs shadow-xl shadow-zinc-200"
          >
      ...
      <section className="glass-panel p-6 border border-zinc-200 bg-zinc-100 w-full rounded-2xl shadow-xl">
        <h3 className="text-center text-xs font-bold font-display text-zinc-700 uppercase tracking-widest mb-8">
          ...
          {workflowSteps.map((step, idx) => (
            <div key={idx} className="flex flex-col items-center text-center relative group">
              {idx < workflowSteps.length - 1 && (
                <div className="hidden md:block absolute top-5 -right-[50%] w-full h-[1px] bg-zinc-100 z-0" />
              )}
              
              <div className="w-10 h-10 rounded-xl bg-zinc-100 border border-zinc-200 flex items-center justify-center font-bold text-zinc-700 font-mono text-xs relative z-10 group-hover:border-zinc-300 group-hover:text-zinc-900 transition-all shadow-lg">
                {idx + 1}
              </div>
              
              <p className="text-xs font-bold text-zinc-900 mt-3 leading-tight">{step.label}</p>
              <p className="text-[10.5px] text-zinc-500 mt-1 leading-normal">{step.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        {features.map((feat, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className="glass-panel p-5 flex gap-4 text-left items-start border border-zinc-200 hover:border-zinc-300 transition-all rounded-2xl"
            >
              <div className={`p-2.5 rounded-xl bg-gradient-to-br ${feat.color} text-zinc-900 shadow-lg shrink-0`}>
                <Icon size={18} />
              </div>
              <div>
                <h4 className="font-display font-bold text-zinc-900 text-sm mb-1">{feat.title}</h4>
      ...
      <section className="w-full text-center">
        <h3 className="text-xl font-bold font-display text-zinc-900 mb-6">Designed for Professional Retention</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
          <div className="glass-panel p-5 bg-zinc-100 border-zinc-200 rounded-2xl">
            <h4 className="text-zinc-500 font-bold font-display text-xs uppercase tracking-wider mb-3">Disconnected Learning</h4>
          ...
          <div className="glass-panel p-5 bg-zinc-100 border-zinc-300 rounded-2xl shadow-lg shadow-zinc-200">
            <h4 className="text-zinc-700 font-bold font-display text-xs uppercase tracking-wider mb-3">CareerOS Ecosystem</h4>
            <ul className="space-y-2 text-xs text-zinc-700">
              <li className="flex items-center gap-2 text-zinc-600 font-medium">✓ Study sessions calculated automatically in meeting gaps.</li>
              <li className="flex items-center gap-2 text-zinc-600 font-medium">✓ Dynamic skills tracking mapped directly to personal roadmaps.</li>
              <li className="flex items-center gap-2 text-zinc-600 font-medium">✓ Critical re-planning alerts protect timelines from slipping.</li>
            </ul>
          </div>
        </div>
      </section>
