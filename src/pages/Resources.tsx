...
export const Resources: React.FC = () => {
  ...
  const getIcon = (type: Resource['type']) => {
    switch (type) {
      case 'video': return <Film size={15} className="text-blue-400" />;
      case 'paper': return <FileText size={15} className="text-indigo-400" />;
      case 'doc': return <Compass size={15} className="text-cyan-400" />;
      case 'book': return <BookOpen size={15} className="text-amber-400" />;
      case 'project': return <Code2 size={15} className="text-emerald-400" />;
      default: return <Link2 size={15} className="text-slate-400" />;
    }
  };
    ...
    <div className="flex flex-col gap-6 w-full pb-12 font-sans text-slate-200">
      ...
      <div className="glass-panel p-6 bg-gradient-to-r from-slate-900/90 via-[#0c0e18]/90 to-slate-900/90 border border-brand-border rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-5 shadow-lg">
        <div className="flex items-start gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-indigo-500/10 border border-cyan-500/30 flex items-center justify-center shrink-0 text-2xl shadow-inner">
            📚
          </div>
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
      ...
      <div className="glass-panel p-4 bg-[#090a12]/80 border border-brand-border rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
        
        {/* Search with AI Trigger */}
        <div className="flex items-center gap-2 w-full md:w-96">
          <div className="flex items-center gap-2 bg-slate-950 border border-brand-border rounded-xl px-3 py-2 w-full focus-within:border-indigo-500/50 transition-all text-xs">
          ...
          >
            <Sparkles size={13} className={isAiCurating ? 'animate-spin text-amber-300' : 'text-amber-300'} />
        ...
        <div className="flex items-center gap-4 text-xs font-mono w-full md:w-auto justify-between md:justify-end">
          <span className="text-slate-400">
            Read Progress: <strong className="text-emerald-400">{completedCount} / {totalCount}</strong> ({completedPercent}%)
          </span>
          <div className="w-28 h-2 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
      ...
      <div className="flex justify-between items-center flex-wrap gap-4 border-b border-brand-border/60 pb-3">
        {/* Type filters */}
        <div className="flex bg-[#06070d] p-1 rounded-xl border border-brand-border/70 overflow-x-auto text-xs">
          {(['all', 'video', 'paper', 'project', 'doc', 'book'] as const).map(filter => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold font-sans uppercase tracking-wider transition-all ${
                activeFilter === filter ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
          ...
          {(['all', 'pending', 'completed'] as const).map(st => (
            <button
              ...
              className={`px-3 py-1 rounded-lg capitalize transition-all ${
                statusFilter === st ? 'bg-slate-800 text-white font-bold' : 'text-slate-400 hover:text-white'
              }`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filteredResources.length > 0 ? (
          filteredResources.map(res => (
            <div 
              key={res.id} 
              className={`glass-panel p-5 flex flex-col justify-between gap-4 bg-[#090b14]/80 border rounded-2xl transition-all duration-300 shadow-md ${
                res.completed 
                  ? 'border-emerald-500/30 bg-emerald-950/10' 
                  : 'border-brand-border/80 hover:border-indigo-500/40 hover:bg-[#0d0f1c]/90'
              }`}
            >
              <div className="flex gap-4 items-start text-left">
                <div className="p-3 rounded-xl bg-slate-950 border border-brand-border/80 shrink-0 mt-0.5 shadow-inner">
                  {getIcon(res.type)}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap text-[9px] font-mono font-bold uppercase tracking-wider text-slate-500">
                    <span className="text-indigo-400 bg-indigo-950/40 px-1.5 py-0.2 rounded border border-indigo-800/40">{res.type}</span>
                    <span>•</span>
                    <span className="flex items-center gap-1"><Clock size={10} />{res.duration}</span>
                    <span>•</span>
                    <span className={res.difficulty === 'Advanced' ? 'text-red-400' : 'text-slate-300'}>{res.difficulty}</span>
                  </div>
                  
                  <h3 className="text-sm font-bold text-white mt-1.5 leading-snug">{res.title}</h3>
                  <p className="text-xs text-slate-400 mt-2 leading-relaxed italic bg-slate-950/40 p-2.5 rounded-lg border border-brand-border/40">
                    "{res.whyRecommended}"
                  </p>
                </div>
              </div>
              ...
              {/* Action Buttons */}
              <div className="border-t border-brand-border/40 pt-3 flex justify-between items-center text-xs font-mono">
                <button
                  ...
                  className={`flex items-center gap-1.5 transition-colors font-bold uppercase tracking-wider text-[10px] ${
                    res.completed ? 'text-emerald-400 hover:text-emerald-300' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {res.completed ? (
                    <>
                    ...
                    <>
                      <Circle size={15} className="text-slate-600 hover:text-indigo-400" />
          ...
          <div className="col-span-full py-16 text-center text-slate-500 italic text-xs bg-slate-950/20 rounded-2xl border border-dashed border-brand-border/40">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="glass-panel p-6 bg-[#0c0d15] border border-brand-border w-full max-w-lg rounded-2xl shadow-2xl">
            <div className="flex justify-between items-center border-b border-brand-border/50 pb-3 mb-4">
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input 
                  ...
                  className="bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-white placeholder-slate-500 w-full focus:outline-none focus:border-indigo-500"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white w-full focus:outline-none focus:border-indigo-500"
                  >
                ...
                <div>
                  <label className="text-slate-400 font-semibold block mb-1">Est. Duration</label>
                  <input 
                    type="text" 
                    value={newDuration}
                    onChange={(e) => setNewDuration(e.target.value)}
                    placeholder="e.g. 45 min"
                    className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white w-full focus:outline-none focus:border-indigo-500 font-mono"
                  />
                </div>
              </div>

              <div>
                <label className="text-slate-400 font-semibold block mb-1">URL Reference</label>
                <input 
                  type="url" 
                  value={newUrl}
                  onChange={(e) => setNewUrl(e.target.value)}
                  placeholder="https://arxiv.org/abs/2307.08691"
                  className="bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-white placeholder-slate-500 w-full focus:outline-none focus:border-indigo-500 font-mono"
                  required
                />
              </div>

              <div>
                <label className="text-slate-400 font-semibold block mb-1">Why Recommended / Notes</label>
                <textarea 
                  value={newWhy}
                  onChange={(e) => setNewWhy(e.target.value)}
                  placeholder="Key GPU SRAM memory IO speedups and parallelization notes..."
                  rows={2}
                  className="bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-white placeholder-slate-500 w-full focus:outline-none focus:border-indigo-500 resize-none"
                />
              </div>
