...
export const CourseDetails: React.FC<CourseDetailsProps> = ({ onNavigate, courseId }) => {
  ...
  if (!course) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        <p className="text-slate-400 mb-4">No course loaded.</p>
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-12 font-sans text-slate-200">
      ...
      {/* HEADER ROW */}
      <div className="glass-panel p-6 bg-gradient-to-r from-slate-900/90 via-[#0c0e18]/90 to-slate-900/90 border border-brand-border rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-5 shadow-xl">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => onNavigate('learning')}
            className="p-2.5 rounded-xl bg-slate-950/80 border border-brand-border text-slate-400 hover:text-white hover:bg-slate-800 transition-all shrink-0 shadow-sm"
          >
            <ArrowLeft size={16} />
          </button>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] text-indigo-300 uppercase tracking-widest font-bold font-mono bg-indigo-950/60 px-2.5 py-0.5 rounded-md border border-indigo-800/40">
                {course.provider || 'University Masterclass'}
              </span>
              <span className="inline-flex items-center gap-1 text-[9px] font-mono font-bold uppercase bg-emerald-950/60 border border-emerald-900/40 text-emerald-400 px-2.5 py-0.5 rounded-full">
                <ShieldCheck size={11} />
                <span>AI Quality Verified</span>
              </span>
            </div>
            <h2 className="text-lg sm:text-2xl font-bold font-sans text-white mt-1.5 leading-snug">{course.title}</h2>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <div className="flex items-center gap-2.5 text-xs font-mono font-semibold bg-slate-950/70 px-3.5 py-2 rounded-xl border border-brand-border/80 shadow-inner">
            ...
            <div className="w-16 h-1.5 bg-slate-900 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 rounded-full transition-all duration-700" style={{ width: `${course.progress}%` }} />
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-800/40 hover:border-emerald-500 text-emerald-400 hover:text-white text-xs font-mono font-bold uppercase transition-all shadow-sm"
          >
            <ShieldCheck size={13} />
            <span>Audit</span>
          </button>
      ...
      {/* =========================================================================
         ...
         ========================================================================= */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 w-full p-2 bg-[#090b16]/95 border border-brand-border/80 rounded-2xl shadow-xl">
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 py-2.5 px-2 rounded-xl text-xs font-bold font-display uppercase tracking-wider transition-all duration-200 text-center ${
            activeTab === 'textbook'
              ? 'bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-600 text-white shadow-lg shadow-indigo-600/35 border border-white/20 scale-[1.02]'
              : 'bg-slate-900/60 hover:bg-slate-800/80 text-slate-300 hover:text-white border border-brand-border/50 hover:border-indigo-500/40'
          }`}
        >
          <span className="text-sm">📖</span>
          <BookOpen size={13} className={activeTab === 'textbook' ? 'text-white' : 'text-indigo-400'} />
        ...
        >
          <span className="text-sm">⭐</span>
          <GithubIcon size={13} className={activeTab === 'github' ? 'text-white' : 'text-amber-400'} />
        ...
        >
          <span className="text-sm">🎥</span>
          <Video size={13} className={activeTab === 'projects' ? 'text-white' : 'text-cyan-400'} />
        ...
        >
          <span className="text-sm">💻</span>
          <Terminal size={13} className={activeTab === 'sandbox' ? 'text-white' : 'text-emerald-400'} />
        ...
        >
          <span className="text-sm">🧠</span>
          <Award size={13} className={activeTab === 'quiz' ? 'text-white' : 'text-yellow-400'} />
        ...
        >
          <span className="text-sm">🤖</span>
          <Sparkles size={13} className={activeTab === 'tutor' ? 'text-white' : 'text-purple-400'} />
      ...
      {activeTab === 'textbook' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="glass-panel p-5 bg-[#0a0c16]/90 border border-brand-border rounded-2xl flex flex-col gap-3 shadow-md">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border flex items-center justify-between gap-3 transition-all ${
                      isCurrent
                        ? 'border-indigo-500 bg-indigo-950/60 text-white shadow-[0_0_15px_rgba(99,102,241,0.25)]'
                        : isCompleted
                          ? 'border-brand-border/70 bg-slate-950/60 text-slate-300 hover:bg-slate-900/60'
                          : 'border-brand-border/40 bg-slate-950/30 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className={`w-5 h-5 rounded-lg flex items-center justify-center font-mono font-bold text-[10px] shrink-0 ${
                        ...
                        isCurrent ? 'bg-indigo-500/30 text-indigo-300 border border-indigo-500/50' :
                    ...
                    <span className={`text-[9px] font-mono shrink-0 uppercase font-bold ${isCompleted ? 'text-emerald-400' : isCurrent ? 'text-indigo-300' : 'text-slate-500'}`}>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="glass-panel p-6 bg-[#0a0c16]/90 border border-brand-border rounded-2xl shadow-md flex flex-col gap-5">
              <div>
                <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                  <span className="text-[10px] font-bold font-mono text-indigo-300 uppercase tracking-widest bg-indigo-950/70 px-2.5 py-0.5 rounded border border-indigo-800/50">
                    Chapter {activeChapterIdx + 1} of {course.chapters.length}
                  </span>
                  <span className="text-xs font-mono text-slate-400">Estimated 45 min deep study</span>
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-white tracking-tight leading-snug">{activeChapter.title}</h3>
              </div>

              {/* =========================================================================
                  INLINE INTERACTIVE VIDEO LECTURE TEMPLATE / PLAYER
                 ========================================================================= */}
              <div className="rounded-2xl border border-brand-border bg-slate-950/90 overflow-hidden shadow-xl flex flex-col">
                {/* Video Top Controls & Stream Switcher */}
                <div className="p-3.5 bg-[#070812] border-b border-brand-border/80 flex items-center justify-between flex-wrap gap-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold font-mono text-white uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      <span className="text-indigo-300 font-normal">({activeLecture.creatorName.split(' ')[0]})</span>
                    </span>
                  </div>

                  <div className="flex items-center gap-2">
                    {/* Dual Stream Switcher */}
                    <div className="flex bg-[#06070d] p-0.5 rounded-xl border border-brand-border/80 text-[10px] font-mono">
                      <button
                        onClick={() => {
                          setLectureSourceType('indian');
                        }}
                        className={`px-3 py-1 rounded-lg font-bold transition-all ${
                          lectureSourceType === 'indian' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                        }`}
                      >
                        🇮🇳 Indian Masterclass
                      </button>
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg font-bold transition-all ${
                          lectureSourceType === 'foreign' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                        }`}
                      >
                        🌍 Global / Stanford
                      </button>
                    ...
                    {isPlayingMainVideo && (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-slate-800 text-slate-300 hover:text-white text-[10px] font-mono font-bold uppercase transition-colors"
                        title="Close Player"
                      >
                        Minimize
                      </button>
                  ...
                  {isPlayingMainVideo ? (
                    ...
                    >
                      ...
                      <div className="absolute top-4 left-4 flex items-center gap-2">
                        <span className="px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold uppercase bg-black/75 text-amber-300 border border-amber-500/30 backdrop-blur-md">
                          {lectureSourceType === 'indian' ? '🇮🇳 Verified Indian Track' : '🌍 University Track'}
                        </span>
                        <span className="px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold uppercase bg-black/75 text-slate-300 border border-slate-700 backdrop-blur-md">
                          HD 1080p Stream
                        </span>
                      </div>

                      {/* Glowing Centered Play Button */}
                      <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-tr from-indigo-600 via-indigo-500 to-violet-500 text-white flex items-center justify-center shadow-[0_0_40px_rgba(99,102,241,0.7)] group-hover:scale-110 group-hover:shadow-[0_0_60px_rgba(99,102,241,0.9)] transition-all duration-300 border-2 border-white/30">
                      ...
                      <div className="absolute bottom-4 left-4 right-4 text-left p-3.5 rounded-xl bg-black/80 backdrop-blur-md border border-white/10 flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-xs sm:text-sm font-bold text-white truncate">{activeLecture.title}</p>
                          <p className="text-[10px] text-indigo-300 font-mono mt-0.5 truncate">
                            {activeLecture.channelName} • {activeLecture.creatorName}
                          </p>
                        </div>
                        <div className="px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-wider font-display shrink-0 flex items-center gap-1.5 shadow-md">
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-bold font-mono text-indigo-300 uppercase tracking-wider">Theory & Mathematical Formulation</h4>
                <p className="text-xs sm:text-sm text-slate-200 leading-relaxed bg-slate-950/60 p-4 rounded-xl border border-brand-border/70 font-normal">
              ...
              {activeChapter.analogy && (
                <div className="p-4 rounded-xl bg-gradient-to-r from-amber-500/10 via-orange-500/5 to-transparent border border-amber-500/30 flex gap-3.5 items-start">
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-amber-300 uppercase tracking-wider font-mono">Intuitive Mental Model</h5>
                    <p className="text-xs text-slate-200 mt-1 leading-relaxed italic">{activeChapter.analogy}</p>
                  </div>
                </div>
              )}
              ...
              {activeChapter.keyTerminology && activeChapter.keyTerminology.length > 0 && (
                <div>
                  <h4 className="text-xs font-bold font-mono text-indigo-300 uppercase tracking-wider mb-2">Key Vocabulary & Concepts</h4>
                  <div className="flex flex-wrap gap-1.5">
                    {activeChapter.keyTerminology.map((term, i) => (
                      <span key={i} className="px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold text-slate-200 bg-slate-950 border border-brand-border hover:border-indigo-500/40 transition-colors">
                        {term}
                      </span>
                    ))}
                  </div>
              ...
              {activeChapter.practiceTask && (
                <div className="p-4 rounded-xl bg-slate-950/80 border border-brand-border flex flex-col gap-2">
                  ...
                  <p className="text-xs text-slate-200 leading-relaxed font-mono bg-[#06070d] p-3 rounded-lg border border-slate-900">
                    {activeChapter.practiceTask}
                  </p>
                </div>
              )}
              ...
              <div className="flex justify-between items-center pt-4 border-t border-brand-border/60">
                ...
                <button
                  ...
                  className="btn-primary py-2 px-5 text-xs font-bold uppercase tracking-wider flex items-center gap-2 shadow-md shadow-indigo-600/30"
                >
      ...
      {activeTab === 'github' && (
        <div className="glass-panel p-6 bg-[#0a0c16]/90 border border-brand-border rounded-2xl shadow-xl flex flex-col gap-6">
          <div className="flex justify-between items-center border-b border-brand-border/60 pb-3 flex-wrap gap-2">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <GithubIcon size={18} className="text-amber-400" />
                <span>Production GitHub Repositories & Real-World Codebases</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Top-starred, open-source reference implementations, inference engines, and production repos for <strong className="text-slate-200">{course.title}</strong>.
              </p>
            </div>

            <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950/60 px-3 py-1 rounded-lg border border-emerald-800/40 font-bold uppercase">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {githubRepos.map((repo, idx) => (
              <div 
                key={idx}
                className="p-5 rounded-2xl border border-brand-border bg-slate-950/80 hover:border-indigo-500/50 hover:bg-[#0c0f1e] transition-all duration-300 flex flex-col justify-between gap-4 shadow-lg group relative"
              >
                <div>
                  {/* Top Bar with Star rating and Language */}
                  <div className="flex justify-between items-start gap-2 mb-2.5">
                    <div className="flex items-center gap-2.5">
                      <div className="p-2.5 rounded-xl bg-slate-900 border border-brand-border text-amber-400 group-hover:scale-105 transition-transform">
                        <GithubIcon size={18} />
                      </div>
                      <div>
                        <a 
                          ...
                          target="_blank" 
                          rel="noreferrer"
                          className="text-sm font-bold text-indigo-300 hover:text-indigo-200 hover:underline flex items-center gap-1.5 leading-tight font-mono"
                        >
                          <span>{repo.name}</span>
                          <ExternalLink size={12} className="opacity-70" />
                        </a>
                        <span className="text-[10px] text-slate-400 font-mono font-bold mt-0.5 inline-block">
                          ● {repo.language || 'Python'}
                        </span>
                      </div>
                    </div>

                    <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30 flex items-center gap-1 shrink-0">
                  ...
                  {repo.topics && (
                    <div className="flex flex-wrap gap-1.5 mt-3.5">
                      {repo.topics.map((t, i) => (
                        <span key={i} className="text-[9.5px] font-mono px-2 py-0.5 rounded-md bg-slate-900 text-slate-300 border border-slate-800">
                          #{t}
                        </span>
                      ))}
                    </div>
                ...
                <div className="pt-3 border-t border-brand-border/60 flex flex-col gap-2.5">
                  <div className="flex items-center justify-between bg-[#06070d] px-3.5 py-2 rounded-xl border border-slate-800 font-mono text-[11px] text-slate-300 overflow-hidden">
                    <span className="truncate text-slate-300">{repo.cloneCommand || `git clone ${repo.url}.git`}</span>
                    <button
      ...
      {activeTab === 'projects' && (
        <div className="glass-panel p-6 bg-[#0a0c16]/90 border border-brand-border rounded-2xl shadow-xl flex flex-col gap-6">
          <div className="flex justify-between items-center border-b border-brand-border/60 pb-3 flex-wrap gap-2">
            ...
            <span className="text-[10px] font-mono text-cyan-400 bg-cyan-950/60 px-3 py-1 rounded-lg border border-cyan-800/40 font-bold uppercase">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {videoProjects.map((proj, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="p-5 rounded-2xl border border-brand-border bg-slate-950/80 hover:border-cyan-500/50 hover:bg-[#0c0f1e] transition-all duration-300 flex flex-col justify-between gap-4 shadow-lg overflow-hidden"
                >
                  <div>
                    {/* Header Metadata */}
                    <div className="flex items-center justify-between gap-2 mb-2.5 text-[10px] font-mono text-slate-400">
                      <span className="text-cyan-400 font-bold uppercase bg-cyan-950/50 px-2.5 py-0.5 rounded border border-cyan-800/40">
                        {proj.channel || 'Verified Lab'}
                      </span>
                      <span className="text-slate-300 font-semibold">{proj.duration || '1h 30m'}</span>
                    </div>

                    <h4 className="text-sm sm:text-base font-bold text-white leading-snug">{proj.title}</h4>

                    {/* Interactive Video Player Canvas inside the Card */}
                    <div className="relative w-full aspect-video rounded-xl overflow-hidden mt-3 bg-black border border-slate-800">
                      {isPlayingThisProj ? (
                        <iframe
                        ...
                        >
                          <div className="w-12 h-12 rounded-full bg-cyan-600/90 text-white flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-200 border border-white/30">
                            <Play size={20} fill="currentColor" className="ml-0.5 text-white" />
                          </div>
                          <span className="mt-2 text-[10px] font-mono font-bold uppercase text-white bg-black/70 px-2.5 py-0.5 rounded backdrop-blur-sm border border-white/10">
                    ...
                    <p className="text-xs text-slate-200 mt-3 leading-relaxed font-normal">{proj.description}</p>

                    {/* Key Concepts */}
                    {proj.keyConcepts && (
                      <div className="mt-3.5">
                        <p className="text-[9.5px] font-mono uppercase font-bold text-slate-400 mb-1.5">Architectures Covered:</p>
                        <div className="flex flex-wrap gap-1.5">
                          {proj.keyConcepts.map((kc, i) => (
                            <span key={i} className="text-[9.5px] font-mono px-2.5 py-0.5 rounded bg-indigo-950/60 text-indigo-300 border border-indigo-900/40 flex items-center gap-1">
                  ...
                  <div className="pt-3.5 border-t border-brand-border/60 flex items-center justify-between gap-3">
                    {proj.githubUrl ? (
                      <a
                        href={proj.githubUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs text-slate-300 hover:text-white font-mono flex items-center gap-1.5 hover:underline"
                      >
                        <GithubIcon size={14} />
                        <span>Repo Code</span>
                      </a>
                    ...
                    {isPlayingThisProj ? (
                      ...
                      <button
                        onClick={() => setPlayingProjectId(projKey)}
                        className="btn-primary py-2 px-4 text-xs font-bold font-sans uppercase tracking-wider flex items-center gap-1.5 shadow-md shadow-indigo-600/30"
                      >
                        <Play size={11} fill="currentColor" />
      ...
      {activeTab === 'sandbox' && (
        <div className="glass-panel p-6 bg-[#0a0c16]/90 border border-brand-border rounded-2xl shadow-xl flex flex-col gap-5">
          <div className="flex justify-between items-center border-b border-brand-border/60 pb-3 flex-wrap gap-2">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                rows={16}
                className="w-full bg-[#05060b] border border-slate-800 rounded-xl p-4 font-mono text-xs text-emerald-400 focus:outline-none focus:border-indigo-500 leading-relaxed shadow-inner"
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="w-full h-full bg-[#05060b] border border-slate-800 rounded-xl p-4 font-mono text-xs text-slate-200 overflow-y-auto leading-relaxed shadow-inner min-h-[300px]">
                {sandboxOutput ? (
      ...
      {activeTab === 'quiz' && (
        <div className="glass-panel p-6 bg-[#0a0c16]/90 border border-brand-border rounded-2xl shadow-xl flex flex-col gap-6">
          <div className="flex justify-between items-center border-b border-brand-border/60 pb-3 flex-wrap gap-2">
            ...
            <span className="text-[10px] font-mono text-amber-400 bg-amber-950/60 px-3 py-1 rounded-lg border border-amber-800/40 font-bold uppercase">
              +30 XP Checkpoint
            </span>
          </div>

          <div className="p-5 rounded-2xl bg-slate-950/80 border border-brand-border flex flex-col gap-4">
            ...
            <div className="space-y-2.5">
              {(activeChapter.quizQuestion?.options || [
                ...
                let optStyle = 'border-brand-border/80 bg-slate-900/60 text-slate-200 hover:border-indigo-500/50 hover:bg-slate-900';
                if (isSelected && !quizSubmitted) {
                  optStyle = 'border-indigo-500 bg-indigo-950/70 text-white shadow-md';
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-5 h-5 rounded-full border border-slate-700 flex items-center justify-center font-mono font-bold text-[10px] text-slate-400 shrink-0">
                        {String.fromCharCode(65 + i)}
                      </span>
                      <span className="leading-snug">{opt}</span>
                    </div>
                    {quizSubmitted && isCorrect && <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />}
                    {quizSubmitted && isSelected && !isCorrect && <X size={16} className="text-rose-400 shrink-0" />}
                  </button>
                );
              })}
            </div>
            ...
            {quizSubmitted && (
              <div className={`p-4 rounded-xl border text-xs leading-relaxed ${
                quizSuccess ? 'bg-emerald-950/40 border-emerald-500/40 text-emerald-300' : 'bg-rose-950/40 border-rose-500/40 text-rose-300'
              }`}>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-brand-border/60">
              {quizSubmitted ? (
                ...
                <button 
                  ...
                  className="btn-primary py-2 px-6 text-xs font-bold uppercase shadow-md shadow-indigo-600/30"
                >
      ...
      {activeTab === 'tutor' && (
        <div className="glass-panel p-6 bg-[#0a0c16]/90 border border-brand-border rounded-2xl shadow-xl flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            <span className="text-[10px] font-mono uppercase text-slate-400 font-bold">Quick Prompts:</span>
            {[
              ...
              <button
                ...
                className="px-3 py-1 rounded-lg text-xs bg-slate-900 border border-brand-border hover:border-indigo-500/50 text-slate-300 hover:text-white transition-all shadow-sm"
              >
                {p}
              </button>
            ))}
          ...
          {/* Chat Messages Log */}
          <div className="h-96 overflow-y-auto space-y-3 p-4 rounded-xl bg-slate-950/80 border border-brand-border/80 text-xs">
            ...
            {isAiResponding && (
              <div className="flex items-center gap-2 text-indigo-400 font-mono text-xs animate-pulse p-2">
                <Sparkles size={13} />
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => handleAskAI()}
              className="btn-primary py-2.5 px-5 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-md shadow-indigo-600/30"
            >
              <Send size={13} />
              <span>Ask</span>
            </button>
      ...
      {showAuditModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="glass-panel p-6 max-w-lg w-full bg-[#0a0c16] border border-brand-border rounded-2xl shadow-2xl flex flex-col gap-4">
            <div className="flex justify-between items-center border-b border-brand-border/60 pb-3">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
            ...
            {isAuditing ? (
              ...
              <div className="flex flex-col gap-4 text-xs">
                <div className="bg-[#0e101d] border border-brand-border rounded-xl p-4 flex flex-col gap-2">
                  <span className="text-[10px] font-mono font-bold uppercase text-indigo-400">Verification Result:</span>
                  <p className="text-sm font-bold text-emerald-400 font-mono">{auditReport.overallScore}% Fully Verified</p>
